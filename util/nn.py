...
def weight(name, shape, init='xavier', range=0.1, stddev=0.01, init_val=None, group_id=0):
    if init_val != None:
...
def convolution(x, k_h, k_w, c_o, s_h, s_w, name, init_w='xavier', init_b=0, stddev=0.01, padding='SAME', group_id=0):
    c_i = _get_shape(x)[-1]
...
def convolution_no_bias(x, k_h, k_w, c_o, s_h, s_w, name, init_w='xavier', stddev=0.01, padding='SAME', group_id=0):
    c_i = _get_shape(x)[-1]
    convolve = lambda i, k: tf.nn.conv2d(i, k, [1, s_h, s_w, 1], padding=padding)
    with tf.variable_scope(name) as scope:
        w = weight('weights', [k_h, k_w, c_i, c_o], init=init_w, stddev=stddev, group_id=group_id)
...
def fully_connected(x, output_size, name, init_w='xavier', init_b=0, stddev=0.01, group_id=0):
...
def fully_connected_no_bias(x, output_size, name, init_w='xavier', stddev=0.01, group_id=0):
    x_shape = _get_shape(x)
    input_dim = x_shape[-1]
